"""
BOT RSI CANALES - BACKTEST v3.2
================================
3 Estrategias en paralelo:
1. EMA_CRUCE (con filtro de divergencia RSI opcional)
2. RSI_CANALES (con filtro de divergencia RSI opcional)
3. RSI_DIVERGENCIA (nueva - basada solo en divergencia precio/RSI)

Mejoras:
- Divergencia alcista regular como filtro y estrategia independiente
- Métricas comparativas entre las 3 estrategias
...
CONFIG = {
    ...
    # DIVERGENCIA RSI
    'div_order': int(os.environ.get('DIV_ORDER', '3')),
    'div_lookback': int(os.environ.get('DIV_LOOKBACK', '25')),
    'div_rsi_max': float(os.environ.get('DIV_RSI_MAX', '40')),
...
def init_logging():
    global LOG_FILE
    log_path = os.environ.get('LOG_FILE', 'backtest_v3.2.txt')
...
def check_ema_cruce(df, idx, config):
    ...
    if config['div_usar_como_filtro']:
        hay_div, info_div = detectar_divergencia_alcista(
            df, df['rsi'].values, order=config['div_order'],
            lookback=config['div_lookback'], rsi_max=config['div_rsi_max']
        )
        if not hay_div:
...
def detectar_ruptura_diagonal_long(rsi_values, diagonal, df, idx,
                                    umbral_ruptura=0.8, volumen_ratio_min=1.2, volumen_lookback=10):
    if not diagonal:
        return False, None
    n = len(rsi_values)
    if n < 3 or idx < 2:
...
def check_rsi_canales(df, idx, config):
    ...
    if config['div_usar_como_filtro']:
        hay_div, info_div = detectar_divergencia_alcista(
            df, rsi_values, order=config['div_order'],
            lookback=config['div_lookback'], rsi_max=config['div_rsi_max']
        )
        if not hay_div:
            return False, "Sin divergencia RSI confirmada", None
...
# ============================================================================
# ESTRATEGIA 3: RSI DIVERGENCIA (NUEVA)
# ============================================================================

def check_rsi_divergencia(df, idx, config):
    if idx < 50:
        return False, "Datos insuficientes", None
...
def backtest_estrategia(df, nombre_estrategia, config):
    ...
    if nombre_estrategia == 'EMA_CRUCE':
        rechazos = {'cruce': 0, 'volumen': 0, 'divergencia': 0, 'cooldown': 0}
    elif nombre_estrategia == 'RSI_CANALES':
        rechazos = {'diagonal': 0, 'ruptura': 0, 'volumen': 0, 'zona_rsi': 0, 'divergencia': 0, 'cooldown': 0}
    else:  # RSI_DIVERGENCIA
        rechazos = {'sin_divergencia': 0, 'volumen': 0, 'cooldown': 0}
    ...
    for i in range(ventana_min, len(df) - 1):
        ...
        if not ok:
            # Clasificar rechazo
            if "cruce" in str(msg).lower():
                rechazos['cruce'] += 1
            elif "volumen" in str(msg).lower():
                rechazos['volumen'] += 1
            elif "divergencia" in str(msg).lower() or "sin divergencia" in str(msg).lower():
                rechazos['divergencia'] += 1
            elif "diagonal" in str(msg).lower():
                rechazos['diagonal'] += 1
            elif "ruptura" in str(msg).lower():
                rechazos['ruptura'] += 1
            elif "rsi" in str(msg).lower() and ">=" in str(msg):
                rechazos['zona_rsi'] += 1
            elif "sin divergencia" in str(msg).lower():
        ...
        if resultado:
            ...
            trade = {
                ...
                'divergencia': info_div is not None,
...
def calcular_metricas(trades, balance_inicial=1000):
    ...
    trades_con_divergencia = [t for t in trades if t.get('divergencia', False)]
    trades_sin_divergencia = [t for t in trades if not t.get('divergencia', False)]
    ...
    return {
        ...
        'trades_con_divergencia': len(trades_con_divergencia),
        'trades_sin_divergencia': len(trades_sin_divergencia),
    }
...
def main():
    ...
    try:
        log("="*60)
        log("BOT RSI CANALES - BACKTEST v3.2")
        ...
        log("\n" + "="*60)
        log("RESULTADOS COMPARATIVOS v3.2")
        log("="*60)
        ...
        json_path = os.path.join(output_dir, 'backtest_v3.2_resultados.json')
        with open(json_path, 'w', encoding='utf-8') as f:
            json.dump(resultados_json, f, indent=2, default=str)
        log(f"\n✓ Resultados guardados en: {json_path}")

        # Detalle trades de la mejor estrategia
        ...
        if trades_mejor:
            ...
            log("-" * 60)
            for i, t in enumerate(trades_mejor[:10], 1):  # Primeros 10 trades
                emoji = "🟢" if t['pnl_pct'] > 0 else "🔴"
                div_mark = " [🔄DIV]" if t.get('divergencia') else ""
                log(f"{emoji} #{i} | {t['fecha_entrada'].strftime('%Y-%m-%d %H:%M')}{div_mark}")
