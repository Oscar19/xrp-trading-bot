"""
BOT RSI CANALES - BACKTEST v3.2.1
==================================
Corrección: RSI_DIVERGENCIA solo detecta divergencias RECIENTES
(pivot más reciente dentro de max_distancia_pivot barras del índice actual)

Estrategias:
1. EMA_CRUCE (con filtro de divergencia RSI opcional)
2. RSI_CANALES (con filtro de divergencia RSI opcional)
3. RSI_DIVERGENCIA (solo divergencias recientes)
"""
...
CONFIG = {
    ...
    'div_max_distancia_pivot': int(os.environ.get('DIV_MAX_DISTANCIA', '10')),
...
def init_logging():
    global LOG_FILE
    log_path = os.environ.get('LOG_FILE', 'backtest_v3.2.1.txt')
...
# ============================================================================
# DIVERGENCIA RSI - CORREGIDA v3.2.1
# ============================================================================
...
def detectar_divergencia_alcista(df, rsi_values, idx_actual, order=3, lookback=25, rsi_max=40, max_distancia_pivot=10):
    """
    Detecta divergencia alcista SOLO si el segundo pivot está cerca de idx_actual.
    CORRECCIÓN v3.2.1: Evita que divergencias antiguas sigan generando señales.
    """
    n = len(df)
    ...
    for i in range(len(price_min_idx) - 1, 0, -1):
        ...
        # CORRECCIÓN: El pivot más reciente debe estar cerca del índice actual
        if (idx_actual - idx2) > max_distancia_pivot:
            continue
        ...
        divergencias.append({
            ...
            'distancia_actual': int(idx_actual - idx2),
...
def check_ema_cruce(df, idx, config):
    ...
    if config['div_usar_como_filtro']:
        hay_div, info_div = detectar_divergencia_alcista(
            df, df['rsi'].values, idx_actual=idx,
            order=config['div_order'], lookback=config['div_lookback'],
            rsi_max=config['div_rsi_max'], max_distancia_pivot=config['div_max_distancia_pivot']
        )
        if not hay_div:
...
def check_rsi_divergencia(df, idx, config):
    ...
    if not hay_div:
        return False, "Sin divergencia alcista reciente", None
    volumen_actual = df['volume'].iloc[idx]
    volumen_media = df['volume'].iloc[max(0, idx - config['volumen_lookback']):idx].mean()
    ratio_volumen = volumen_actual / volumen_media if volumen_media > 0 else 0
    if ratio_volumen < config['volumen_min_ratio']:
...
def backtest_estrategia(df, nombre_estrategia, config):
    ...
    if nombre_estrategia == 'EMA_CRUCE':
        ...
        rechazos.update({'sin_divergencia': 0, 'volumen': 0})
...
def main():
    ...
    try:
        log("="*60)
        log("BOT RSI CANALES - BACKTEST v3.2.1")
        log("CORRECCION: Divergencia solo reciente (max_distancia_pivot)")
        ...
        log(f"Max distancia pivot: {CONFIG['div_max_distancia_pivot']}")
        ...
        log("\\n" + "="*60)
        log("RESULTADOS COMPARATIVOS v3.2.1")
        log("="*60)
        ...
        json_path = os.path.join(output_dir, 'backtest_v3.2.1_resultados.json')
        ...
        if trades_mejor:
            ...
            for i, t in enumerate(trades_mejor[:10], 1):
                ...
                if t.get('divergencia_info'):
                    ...
                    log(f"   Divergencia: Precio ${d['precio1']:.4f}→${d['precio2']:.4f} | RSI {d['rsi1']:.1f}→{d['rsi2']:.1f} | Dist:{d['distancia_actual']}")
                log("")

        log("="*60)
        log("Backtest v3.2.1 finalizado")
        log("="*60)
    finally:
        close_logging()
